...
When a grant selects a team, its ID token and token introspection data include `team_role`. Use this claim together with the granted scopes, not instead of them. Account Chief only authorizes connected apps for Owner and Member memberships; Billing memberships are not available for OAuth team selection.

If the user leaves the selected team or changes to Billing, credentials tied to that team stop working. Handle an API `401` or an `invalid_grant` refresh response by clearing the connection and asking the user to authorize again.
