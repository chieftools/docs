...
Scopes limit what an access token can do. Choose them when you create an [access token](/api/tokens#access-tokens). The general [scopes guide](/api/scopes) explains how Chief Tools applies scopes across its APIs.
...
The token can read domains, contacts, DNS records, TLDs, and activity. It cannot change them.
