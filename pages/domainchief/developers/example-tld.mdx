...
Domain Chief runs its normal application workflows for these domains. The Mock Registry supplies the provider response, so no request reaches a real registry and the domain will not resolve on the public internet.

## Scenario inputs

The Mock Registry recognizes these reserved inputs:
...
The availability check reports the name as active. Submit it through the normal registration endpoint with an authorization code to create an incoming transfer.
...
3. Wait for the transfer to fail. The failure cause is `authorization_code_incorrect`.
4. Retry from the domain page or call `POST /v1/domains/{domainNameOrId}/transfer/retry` with another authorization code.

The retry succeeds and the Mock Registry clears the submitted code from the domain.
...
New `.example` registrations receive a renewal date one month in the future. At the next provider sync after that date, the Mock Registry moves the renewal date forward one month when auto-renew is enabled. If auto-renew is disabled, it marks the domain as expired instead.
